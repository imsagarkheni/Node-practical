...
router.post('/sendotp', async (req, res) => {
  const {fullName,userName, contactNo, countryCode } = req.body;
      ...
      if(userdata) {
          if(timecalculation.timedifferenceinminutes(Date.now(), userdata.otp_timestamp) <= 2){
              if(req.body.otp.toString() == userdata.last_sent_otp){
                  ...
                  await primary.model(constants.MODELS.users, usersModel).findByIdAndUpdate(userdata._id, { status: "Online" }).lean();
